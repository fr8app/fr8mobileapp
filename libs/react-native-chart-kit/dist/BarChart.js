...
import { Dimensions, View,Text as ReactText } from "react-native";
import { Defs, G, LinearGradient, Rect, Stop, Svg, Text,Polygon } from "react-native-svg";
...
var BarChart = /** @class */ (function (_super) {
    ...
    function BarChart() {
        ...
        _this.renderBars = function (_a) {
            var data = _a.data,barPress=_a.barPress, width = _a.width, height = _a.height, paddingTop = _a.paddingTop, paddingRight = _a.paddingRight, barRadius = _a.barRadius, withCustomBarColorFromData = _a.withCustomBarColorFromData;
            ...
            return data.map(function (x, i) {
                ...
                onPressIn={(e)=>barPress(e,x,i,(paddingRight +
                    (i * (width - paddingRight)) / data.length +
                    barWidth / 1)-20,(((baseHeight - barHeight) / 4) * 3 + paddingTop - 1)-32)} key={Math.random()}
                // onPressOut={()=>barPress(e,null,0)}
                x={paddingRight +
                    (i * (width - paddingRight)) / data.length +
        ...
        {(_b = dataset.colors) === null || _b === void 0 ? void 0 : _b.map(function (color, colorIndex) {
              ...
              {flatColor ? (<Stop offset="1" stopColor={highOpacityColor} stopOpacity="0.5"/>) : (<Stop offset="1" stopColor={lowOpacityColor} stopOpacity="0"/>)}
            </LinearGradient>);
                })}
      </Defs>);
            });
            ...
            return data.map(function (x, i) {
                ...
                <Svg >
                    ...
                    x={(paddingRight +
                        (i * (width - paddingRight)) / data.length +
                        barWidth / 1)-20} 
                        y={(((baseHeight - barHeight) / 4) * 3 + paddingTop - 1)-32} 
                    fill={'rgba(0,0,0,0.7)'}
                    width="100"
                    height="28"
                  />
                <Text 
            //     style={{position:'absolute',textAlign:'center',left:-
            //     paddingRight +(i * (width - paddingRight)) / data.length +barWidth/2 / 1 ,bottom:-((baseHeight - barHeight) / 4) * 3 ,fontSize:10
            // }}
                 key={Math.random()} x={paddingRight +
                    (i * (width - paddingRight)) / data.length +
                    barWidth / 1} y={(((baseHeight - barHeight) / 4) * 3 + paddingTop - 1)-15} fill={_this.props.chartConfig.color(0.6)} fontSize="10" textAnchor="middle"
                    >
          {renderLabel(data[i])}
        </Text>
        <Polygon
                    points="15,0 10,12 0,0"
                    fill={'rgba(0,0,0,0.7)'}
                    x={(paddingRight +
                        (i * (width - paddingRight)) / data.length +
                        barWidth / 1)-5}
                         y={(((baseHeight - barHeight) / 4) * 3 + paddingTop - 1)-4}
                  />
        </Svg>
        );
            });
    ...
    BarChart.prototype.render = function () {
        ...
        <Svg height={height} width={width}>
          ...
          <G>
            {withVerticalLabels
            ? this.renderVerticalLabels(__assign(__assign({}, config), { labels: data.labels, paddingRight: 0, paddingTop: paddingTop, horizontalOffset: barWidth * this.getBarPercentage() }))
            : null}
          </G>
          <G>
            {this.renderBars(__assign(__assign({}, config), { data: data.datasets[0].data, paddingTop: paddingTop, paddingRight: 0, withCustomBarColorFromData: withCustomBarColorFromData,barPress:barPress }))}
          </G>
          <G>
            {showValuesOnTopOfBars &&
            this.renderValuesOnTopOfBars(__assign(__assign({}, config), { data: data.datasets[0].data, paddingTop: paddingTop, paddingRight: 0 }))}
          </G>
          <G>
            {showBarTops &&
