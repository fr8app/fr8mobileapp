...
<TouchableOpacity
    ...
    let id =
      this.state.key == "add" || this.state.key == "added"
        ? navigation.state?.params?.item
          ? navigation.state?.params?.item?.id
            ? navigation.state?.params?.item?.id
            : navigation.state?.params?.item?.friendInfo?.id
          : navigation.state?.params?.item?.friendInfo?.id
        :
        this.props?.Profile?.profileDetail ?
          this.props?.Profile?.profileDetail?._id :
  ...
  submitButtonReportUser = () => {
    ...
    if (
      ...
      if (route.params) {
        let id = navigation?.state?.params?.item ? navigation?.state?.params?.item?.id : navigation?.state?.params?.userDetail?._id
...
//timeline
  _renderItemTimeline = ({ item, index }) => {
    ...
    return (
      ...
      ratio={2}
        ...
        dis={item.distance == 0 ? '0' : ((item.distance / 1000)*0.621371).toFixed(2)}
        minute={this.getTime(item.minute)}
        terminal={item?.type=='terminal'? item?.terminal?.terminal_name:item.name}
        // EstimatedTime={getTerminalWaitingTime(item)}
        onPress={() => {
