...
export default class loginPopUp extends Component {
  ...
  modalOpen = (name = '', data = null, screen, text, navigation = null, verify = false) => {
    this.getDetail()
    this.seconds = 59
    console.log('data on modal open', text, data, screen);
    this.setState({ navigation: navigation, visible: true, userName: name, data: data, otpVerified: verify, screen, text: text })
    _BackgroundTimer.clearInterval(this.interval)
    this.interval = _BackgroundTimer.setInterval(() => {
  ...
  skip = () => {
    if (this.state.screen == 'addProfile') {
      this.state.navigation.goBack()
    }
    _BackgroundTimer.clearInterval(this.interval)
    this.seconds = 0
    ...
    NetInfo?.fetch()?.then(state => {
      if (state?.isConnected) {
        this.props.ProfileDataInitate(NavigationService.getFullRoute()?._navigation)
      }
    })
  }
  ...
  userExist = () => {
    if (this.state.userName == "" || this.state.userName?.trim().length == 0) {
      ...
      this.props.isUserExistss(this.state.userName, '', this.state.text)
    }
  }
  otpVerify = () => {
    if (this.state.code == "" || this.state.code?.trim().length == 0) {
      ...
      DataManager.getFavList().then((res) => {
        let listParse = JSON.parse(res)
        this.props.verifyOtpForPassword(this.state.code?.trim(), this.state.userName, listParse)
      })
    }
  }
      ...
      <Modal onRequestClose={() => this.modalClose()} transparent={true} ref={this.props.ref} visible={this.state.visible}>
        <View style={{ backgroundColor: 'rgba(0,0,0,0.6)', flex: 1, alignItems: 'center', justifyContent: 'center' }}>
          <View style={{ shadowColor: 'gray', shadowOpacity: 1, shadowRadius: 8, alignItems: 'center', justifyContent: 'center', paddingVertical: 40, shadowOffset: { width: 1, height: 1 }, backgroundColor: '#E3F5FF', width: '90%', borderRadius: 50 }}>
            ...
            <Text style={{ textAlign: 'center', width: '85%', paddingHorizontal: 10, color: '#585353', fontFamily: fontFamily.semiBold }}>
              {this.state.text ? this.state.text : `It's time to Track current Wait Times!\n Setting up your profile allows you to post, see current wait times, share your detention time, and unlock other features that work for you`}
            </Text>
            {this.state.otpVerified ?
              <>
                ...
                <TextInput
                  ...
                  placeholder={'Confirm Password'} style={{ marginTop: 15, paddingVertical: 15, paddingHorizontal: 10, backgroundColor: '#FFFFFF', borderRadius: 30, width: '85%' }} />

                <View style={{ flexDirection: 'row', justifyContent: 'space-between', width: '85%' }}>

                  <TouchableOpacity
                    onPress={() => { this.setPassword() }}
                    style={{ alignSelf: 'center', alignItems: 'center', justifyContent: 'center', marginBottom: 4, width: '45%', paddingVertical: 15, marginTop: 30, borderRadius: 30, justifyContent: 'center', backgroundColor: '#28A2E1' }}>
                    <Text
              ...
              <>
                <TextInput
                  ...
                  placeholder={'Username'} style={{ color: 'lightgray', marginTop: 30, paddingVertical: 15, paddingHorizontal: 10, backgroundColor: '#FFFFFF', borderRadius: 30, width: '85%' }} />
                {this.state.data?.passwordSet == false && this.state.data?.usernameExits == true ?
                  <>
                    <OTPInputView
                      ...
                      codeInputFieldStyle={{
                        fontFamily: fontFamily.museo_500,
                        fontSize: 18,
                        alignSelf: "center",
                        textAlign: "center",
                        borderRadius: 5,
                    ...
                    />
                    <Text style={[{ fontSize: 16, color: 'black', fontFamily: fontFamily.regular, textAlign: 'center' }]} onPress={() => {
                  ...
                  <>
                    <TextInput
                      value={this.state.password}
                      returnKeyType='next'
                      onSubmitEditing={() =>!this.state.data?.usernameExits? this.inputs3.focus():Keyboard.dismiss()}
                      ref={input => this.inputs2 = input}
                      secureTextEntry
                      onChangeText={(t) => {
                        let p = this.removeEmojis(t, true)
                      ...
                      placeholder={'Password'} style={{ marginTop: 15, paddingVertical: 15, paddingHorizontal: 10, backgroundColor: '#FFFFFF', borderRadius: 30, width: '85%' }} />
                    {!this.state.data?.usernameExits && <TextInput
                      value={this.state.cpassword}
                      returnKeyType='done'
                      onSubmitEditing={() => Keyboard.dismiss()}
                      ref={input => this.inputs3 = input}
                ...
                {this.state.data?.passwordSet == false && this.state.data?.usernameExits == true ?
                  <TouchableOpacity
                    ...
                    style={{ alignSelf: 'center', alignItems: 'center', justifyContent: 'center', width: '85%', marginBottom: 4, paddingVertical: 15, marginTop: 30, borderRadius: 30, justifyContent: 'center', backgroundColor: '#28A2E1' }}>
                    <Text
                      style={{ fontSize: 16, fontFamily: fontFamily.bold, color: 'white' }}
                    >{I18n.t('Submit')}</Text>
                  </TouchableOpacity>
                  :
                  <TouchableOpacity
                    onPress={() => { !this.state.data?.usernameExits ? this.registerPress() : this.loginPress() }}
                    style={{ alignSelf: 'center', alignItems: 'center', justifyContent: 'center', width: '85%', marginBottom: 4, paddingVertical: 15, marginTop: 30, borderRadius: 30, justifyContent: 'center', backgroundColor: '#28A2E1' }}>
                    <Text
                      style={{ fontSize: 16, fontFamily: fontFamily.bold, color: 'white' }}
                    >{!this.state.data?.usernameExits ? I18n.t('Register') : I18n.t('Login')}</Text>
                  </TouchableOpacity>
                }
              </>
            }
