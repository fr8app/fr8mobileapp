...
class ManualTimeline extends Component {
    ...
    submitPress = () => {
        ...
        if (!this.nameRef.state.value) {
            ...
            // alert('Timeline location address should be at least 2 characters long.')
        }
                                        ...
                                        if (x.type == 'video/mp4') {
                                            ...
                                            uploadImageOnS3(x.thumbnail, uuidv4_34(), 'route_post', []).then((res) => {
                                                ...
                                                s3bucket.upload(res, (err, data2) => {
                                                    ...
                                                    console.log("Response URL thumbnail : ", data2);
                                                    if (data2.Location) {
                                                        console.log('gdrjkghdhfjkg', data2.Location);
    ...
    //     if(this.state.addedPress != nextState.addedPress) {
    //         return false
    //    }
    //    return true     }
                                ...
                                if (buttonIndex == 1) {
                                    ...
                                    launchImageLibrary(options, (res) => {
                                        if (res.didCancel) {
                                            ...
                                            if (res.assets[0].duration > 15) {
                                                alert(I18n.t('vedioLimit').replace('30', '15'))
                                            }
                                            else {
                                                let response = res.assets[0]
                                                response.path = response.uri
                ...
                if (result == 'granted') {
                    request(PERMISSIONS.ANDROID.READ_EXTERNAL_STORAGE).then((result) => {
                        ...
                        if (result == 'granted') {
                            ...
                            //                 images.push({ thumbnail: f, uri: response.uri, mediaType: type, type: 'image/png', name: response.filename, height: response.height, width: response.width })
                            //                 this.setState({ images: images })
                            //             }).catch((e) => {
